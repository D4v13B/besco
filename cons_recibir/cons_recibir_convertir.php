...
if ($tipo == 1) {
	...
	//Verificarmos el tipo para poder extraer todos los que tengan
	$condicion_factura = $tipo != 1 ? " AND orcd_restante > 0" : "";
}
...
(SELECT '$fact_id', prod_id, $campo_cantidad, orcd_precio, 
		IF(orcd_con_itbms=1, $campo_cantidad*orcd_precio*0.07,0) itbms FROM cons_orden_compra_detalles WHERE orco_id='$orco_id' $condicion_factura)";
mysql_query($qsql);
...
if ($tipo == 2) {
	$qsql = "UPDATE cons_orden_compra_detalles SET 
		orcd_recibido = orcd_recibido + orcd_restante,
		orcd_restante = 0
