...
<th class=tabla_datos_titulo>DISPONIBLES<BR>(PRESUPUESTO)</th>
...
<tbody>
<?php
$coso_id=$_GET['coso_id'];

$qsql ="SELECT  coru_nombre, cosd_cantidad, cosd_id, 
(SELECT copr_nombre FROM cons_proveedores WHERE copr_id=a.copr_id) proveedor,
(SELECT COALESCE(SUM(copr_pendientes),0) FROM construccion_presupuesto WHERE coru_id=a.coru_id AND proy_id=c.proy_id) disponible
FROM cons_solicitudes_detalles a, construccion_rubros b, cons_solicitudes c, proyectos d
WHERE a.coru_id=b.coru_id
AND c.proy_id=d.proy_id
AND coso_id='$coso_id'
AND a.coso_numero=c.coso_numero";
...
<td class=tabla_datos><?php echo mysql_result($rs, $i, 'proveedor'); ?></td>
...
<td><a href="javascript:editar_detalle(<?php echo mysql_result($rs, $i, 'cosd_id')?>)"><img src="imagenes/modificar.png" style="width:20px;height:20px"></a></td>
