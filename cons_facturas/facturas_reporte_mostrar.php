...
<table class="nicetable_th" id="resultado" style="width:99%" class="px-1">
...
<th class=tabla_datos_titulo>No. Orden de Compra</th>
...
$qsql ="SELECT a.faes_id, fact_id, faes_nombre, fact_numero, fact_fecha, copr_nombre, fact_subtotal, fact_impuesto, fact_total, fact_saldo, d.orco_numero

FROM cons_facturas a, cons_proveedores b, cons_facturas_estados c, cons_orden_compra d

WHERE a.copr_id=b.copr_id 

AND a.faes_id=c.faes_id 

AND a.orco_id=d.orco_id
...
<td class=tabla_datos_iconos>
	<?php if(mysql_result($rs, $i, 'faes_id') != 4):?>
		<a href='javascript:cancelar(<?php echo mysql_result($rs, $i, 'fact_id'); ?>)';><img src='https://cdn-icons-png.flaticon.com/512/7866/7866396.png' border=0 style="width:20px;height:20px" title="Cancelar factura"></a>
	<?php endif?>
</td>
